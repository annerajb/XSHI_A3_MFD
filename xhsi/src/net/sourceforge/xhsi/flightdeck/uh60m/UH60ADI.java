...
import net.sourceforge.xhsi.flightdeck.pfd.*;
...
public class UH60ADI extends UH60MSubcomponent {
    ...
    public UH60ADI(ModelFactory model_factory, UH60MGraphicsConfig empty_gc, Component parent_component) 
    {
        super(model_factory, empty_gc, parent_component);
    }
    ...
    private void drawADI(Graphics2D g2) {
        ...
        int cx = empty_gc.adi_cx;
        int cy = empty_gc.adi_cy;
        int left = empty_gc.adi_size_left;
        int right = empty_gc.adi_size_right;
        int up = empty_gc.adi_size_up;
        int down = empty_gc.adi_size_down;
        int p_90 = empty_gc.adi_pitch90;
        int scale = empty_gc.adi_pitchscale;
...
//pitch = 11.75f;
        ...
        if ( ! colorgradient_horizon ) {
            ...
            if ( empty_gc.draw_hsi ) {
                g2.clipRect(empty_gc.panel_rect.x, empty_gc.panel_rect.y, empty_gc.panel_rect.width, empty_gc.dg_cy - empty_gc.dg_radius - empty_gc.hsi_tick_w - empty_gc.line_height_xl*3/2 - empty_gc.panel_rect.y);
            } else {
//                g2.clipRect(empty_gc.panel_rect.x, empty_gc.tape_top - 1, empty_gc.panel_rect.width, empty_gc.tape_height + 2);
                g2.clipRect(empty_gc.panel_rect.x, empty_gc.panel_rect.y + empty_gc.panel_offset_y, empty_gc.panel_rect.width, empty_gc.panel_rect.width);
            }
        }
        ...
        int diagonal = colorgradient_horizon ?
            (int)Math.hypot( Math.max(cx, empty_gc.panel_rect.width - cx), Math.max(cy, empty_gc.panel_rect.height - cy) ) :
        ...
        if ( colorgradient_horizon ) 
        {
            ...
            GradientPaint up_gradient = new GradientPaint(
                    cx - diagonal, pitch_y - p_90, empty_gc.background_color,
                    cx - diagonal, pitch_y - p_90/2, empty_gc.sky_color,
                    false);
            GradientPaint sky_gradient = new GradientPaint(
                    cx - diagonal, pitch_y - p_90/2, empty_gc.sky_color,
                    cx - diagonal, pitch_y, empty_gc.brightsky_color,
                    false);
            GradientPaint ground_gradient = new GradientPaint(
                    cx - diagonal, pitch_y, empty_gc.brightground_color,
                    cx - diagonal, pitch_y + p_90/2, empty_gc.ground_color,
                    false);
        ...
        if ( Math.abs(bank) > 35.0f ) {
            if ( Math.abs(bank) > 70.0f ) {
                g2.setColor(empty_gc.warning_color);
            } else {
                g2.setColor(empty_gc.caution_color);
            }
        ...
        if ( ! this.aircraft.on_ground() ) 
        {
            ...
            if ( (Math.abs(dx) < down) && (Math.abs(dy) < down) ) {
                ...
                int fpv_y = cy + dy;
                int fpv_r = down/20;
                g2.setColor(empty_gc.fpv_color);
                g2.setStroke(new BasicStroke(3.0f * empty_gc.grow_scaling_factor));
        ...
        if ( ! this.preferences.get_single_cue_fd() ) 
        {
            ...
            int wing_t = Math.round(4 * empty_gc.grow_scaling_factor);
            int wing_i = left / 3;
            int wing_o = left * 7 / 8;
            int wing_h = down / 8;
            int left_wing_x[] = {
...
//if ( true ) {
            ...
            g2.setColor(empty_gc.heading_bug_color);
            ...
            if ( this.preferences.get_single_cue_fd() ) {
                ...
                g2.setColor(empty_gc.instrument_background_color);
                ...
                g2.setStroke(new BasicStroke(3.0f * empty_gc.scaling_factor));
                // hor
...
//        g2.setColor(empty_gc.instrument_background_color);
//        g2.fillRect(empty_gc.border_left + ( empty_gc.frame_size.width - empty_gc.border_left - empty_gc.border_right ) / 32, empty_gc.border_top + ( empty_gc.frame_size.height - empty_gc.border_top - empty_gc.border_bottom ) / 8, ( empty_gc.frame_size.width - empty_gc.border_left - empty_gc.border_right ) / 8, ( empty_gc.frame_size.height - empty_gc.border_top - empty_gc.border_bottom ) / 8 * 6);
...
//    }
    ...
    private void drawPitchmark(Graphics2D g2, float pitch, int pitchmark, int p_y, int p_90, int cx, int cy, int size) {
        ...
        if ( ( p_m <= 900 ) && ( p_m != 0 ) && ( p_m >= -900 ) ) {
            ...
            if ( ( p_m != 0 ) && ( p_m % 100 == 0 ) ) {
                ...
                int f_w = empty_gc.get_text_width(g2, empty_gc.font_s, "00");
                int f_y = empty_gc.line_height_s / 2 - 2;
    ...
    private void drawMarker(Graphics2D g2) {
        ...
        if ( this.avionics.outer_marker() || this.avionics.middle_marker() || this.avionics.inner_marker() ) {
            ...
            int m_r = empty_gc.adi_size_right*2/16;
            int m_x;
            int m_y;
            if ( this.preferences.get_draw_fullwidth_horizon() ) {
                m_x = empty_gc.adi_cx - empty_gc.adi_size_left;
                m_y = m_y = empty_gc.adi_cy - empty_gc.adi_size_up;
            } else {
                m_x = empty_gc.adi_cx + empty_gc.adi_size_right - empty_gc.adi_size_right*1/16 - 2*m_r;
                m_y = empty_gc.adi_cy - empty_gc.adi_size_up + empty_gc.adi_size_right*1/16;
            }
            ...
            Stroke original_stroke = g2.getStroke();
            g2.setStroke(new BasicStroke(4.0f * empty_gc.grow_scaling_factor));
            g2.drawOval(m_x, m_y, 2*m_r, 2*m_r);
            g2.setStroke(original_stroke);

            g2.setFont(empty_gc.font_m);
            g2.drawString(mstr, m_x + m_r - empty_gc.get_text_width(g2, empty_gc.font_m, mstr)/2, m_y + m_r + empty_gc.line_height_m/2 - 2);
